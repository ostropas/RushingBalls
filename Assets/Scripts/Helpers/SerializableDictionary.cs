...
public abstract class SerializableDictionaryBase {
	...
	protected class DerivedDictionary<TKey, TValue> : Dictionary<TKey, TValue> {
		public DerivedDictionary() { }

		public DerivedDictionary(IDictionary<TKey, TValue> dict) : base(dict) { }

		public DerivedDictionary(SerializationInfo info, StreamingContext context) : base(info, context) { }
	}
}

[Serializable]
public abstract class SerializableDictionaryBase<TKey, TValue, TValueStorage> : SerializableDictionaryBase, IDictionary<TKey, TValue>, IDictionary, IReadOnlyDictionary<TKey, TValue>, ISerializationCallbackReceiver, IDeserializationCallback, ISerializable {
	private DerivedDictionary<TKey, TValue> m_dict;
	[SerializeField] private TKey[] m_keys;
	[SerializeField] private TValueStorage[] m_values;

	public SerializableDictionaryBase() {
		m_dict = new DerivedDictionary<TKey, TValue>();
	}

	public SerializableDictionaryBase(IDictionary<TKey, TValue> dict) {
		m_dict = new DerivedDictionary<TKey, TValue>(dict);
	}
	...
	public void CopyFrom(IDictionary<TKey, TValue> dict) {
		m_dict.Clear();
		foreach (KeyValuePair<TKey, TValue> kvp in dict) m_dict[kvp.Key] = kvp.Value;
	}
	...
	public void OnAfterDeserialize() {
		if (m_keys != null && m_values != null && m_keys.Length == m_values.Length) {
			m_dict.Clear();
			int n = m_keys.Length;
			for (int i = 0; i < n; ++i) m_dict[m_keys[i]] = GetValue(m_values, i);
	...
	protected SerializableDictionaryBase(SerializationInfo info, StreamingContext context) {
		m_dict = new DerivedDictionary<TKey, TValue>(info, context);
	}
